...
static __inline int decode_subframe(FLACContext *s, int channel, int32_t* decoded)
{
    ...
    type = get_bits(&s->gb, 6); /* determine block type */
...
int flac_decode_frame(FLACContext *fc, uint8_t *buf, int buf_size, int32_t *wavbuf, size_t* pcountDecoded){
    ...
    /* note: the value range of wavbuf follows the bit-depth of each sample, i.e. 16bit -> +-32767*/
    ...
    *pcountDecoded = (fc->blocksize - sampleCnt) * fc->channels;
    return 0;
}
