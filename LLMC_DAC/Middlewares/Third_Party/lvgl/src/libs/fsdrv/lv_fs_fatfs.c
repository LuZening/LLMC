...
#if LV_USE_FS_FATFS
#include "ff.h" // imported by STM32 CUBE as a middleware
 ...
 */
// TODO: convert const char * path to UTF16
// USER BEGIN
static TCHAR bufU16[_MAX_LFN];
static const size_t COUNT_BUF_U16 = _MAX_LFN;
...
#if _LFN_UNICODE
    // UTF8toUTF16
    size_t sizePath8 = strnlen(path, LV_FS_MAX_PATH_LENGTH);
    // implement UTF8 to UTF16 conversion to be compatible with FatFS
    size_t countConv = UTF8ToUTF16((UTF8*)path, (UTF8*)(path + sizePath8), (UTF16*)bufU16, (UTF16*)(bufU16 + COUNT_BUF_U16));
    FRESULT res = f_open(f, bufU16, flags);
#else
...
static void * fs_dir_open(lv_fs_drv_t * drv, const char * path)
{
    ...
    size_t sizePath8 = strnlen(path, LV_FS_MAX_PATH_LENGTH);
    size_t countConv = UTF8ToUTF16((UTF8*)path, (UTF8*)(path + sizePath8), (UTF16*)bufU16, (UTF16*)(bufU16 + COUNT_BUF_U16));
    FRESULT res = f_opendir(d, bufU16);
#else
...
static lv_fs_res_t fs_dir_read(lv_fs_drv_t * drv, void * dir_p, char * fn, uint32_t fn_len)
{
    ...
    do {
        ...
        // TODO: convert fno.fname from TCHAR to utf8

        const char* fname;
#if _LFN_UNICODE
        size_t countFname = strnlen_TCHAR(fno.fname, _MAX_LFN);
        // borrowed bufU16 to store UTF8
        size_t countU8 = UTF16ToUTF8(fno.fname, fno.fname + countFname, (uint8_t*)bufU16, ((uint8_t*)bufU16) + COUNT_BUF_U16 * 2);
        fname = (char*)bufU16;
        fn_len = countU8;
#else
        fname = fno.fname;
        ...
        if(fno.fattrib & AM_DIR) {
            lv_snprintf(fn, fn_len, "/%s", fname);
        }
        else lv_strncpy(fn, fname, fn_len);
