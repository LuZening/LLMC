...
static lv_obj_t* btn_player_main = NULL; /* for toggling disabled state after USB mode switch */

void refresh_main_player_button(void)
{
    if (!btn_player_main) return;
#ifndef LVGL_SIM
    if (cfg.usbCfgDescSelector == USB_CFG_DESC_MASS_STORAGE)
        lv_obj_set_state(btn_player_main, LV_STATE_DISABLED, 1);
    else
        lv_obj_set_state(btn_player_main, LV_STATE_DISABLED, 0);
#endif
}
