...
{
    // simply copy values aside to up sample

    float32_t *end = pSrc + countAllChs;
    uint16_t countEachCh = countAllChs / nCh;
    uint16_t countEachCh_up = countEachCh * L;
    ...
    for(int i = 0; i < nCh; ++i)
    {
        ...
        if(countRemian > 0)
        {
            for(int j = 0; j < countEachCh; ++j)
            {
                for(int k =0; k < L; ++k)
                {
                    *(pChDstBuf_begin++) = *(pChSrcBuf_begin);
                }
                pChSrcBuf_begin++;
            }
        }
...
size_t pad_i16_to_i32(int32_t* __restrict pDst, const int16_t* __restrict pSrc, size_t bytes, uint8_t oversample, uint8_t nch)
{
    ...
    if(oversample <= 1)
    {
        while(pSrc < pEnd)
        {
            // left justify each 16-bit sample independently
            int32_t v = (int32_t)((uint32_t)(*pSrc) << 16U);
            *(pDst++) = v;
            pSrc++;
        }
        return (bytes << 1);
