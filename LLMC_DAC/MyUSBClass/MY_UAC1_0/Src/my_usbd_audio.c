...
{
	USBD_AUDIO_HandleTypeDef *haudio;
	const size_t STANDARD_COUNT_DIFF = 96;
	const size_t MAX_COUNT = STANDARD_COUNT_DIFF + STANDARD_COUNT_DIFF / USB_MAX_IN_PACKET_SIZE_EXPANSION_DENOMINATOR;
...
#endif
	uint32_t sizeToGet = countDiff_limited * sizePerSampleCount;
	uint32_t sizeGot = 0;
	...
	{
		if(sizeAvail >= (sizePreheatBytes_USB_record)) // fifo is quater filled, ready for read
			ready_for_output_USB_record = 1;
	}

	if(ready_for_output_USB_record)
	{
		// FIFO has sufficient samples
		// FIFO too full, must speed up USB absorption
		if(sizeFree < (fifo_to_USB_record.size >> 3))
		{
			sizeToGet = MIN(countDiff_limited + 1, max_count) * sizePerSampleCount;
		}
		// FIFO too exhuast, must slow down USB absorption
		else if(sizeAvail < (sizePreheatBytes_USB_record >> 2))
		{
			if(countDiff_limited > 1)
				sizeToGet = (countDiff_limited - 1) * sizePerSampleCount;
			else
				sizeToGet = (countDiff_limited) * sizePerSampleCount;
		}

		// FIFO is exhaust
		if (sizeAvail < sizeToGet)
		{
			sizeGot = sizeAvail;
		}
		else
	...
	if (sizeGot < sizeToGet)
	{
		memset(buf + sizeGot, 0, sizeToGet - sizeGot);
...
static uint8_t USBD_AUDIO_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
	...
	if (epnum == (AUDIO_MIC_IN_EP & (0x0F)))
	{
		...
		if(lastSampleCount_mic.state == PRECISE_COUNTER_INIT) // precise counter need an init
		{
			...
			memset(bufEPin, 0, AUDIO_TOTAL_BUF_SIZE);
			USBD_LL_FlushEP(pdev, AUDIO_MIC_IN_EP);
...
static uint8_t USBD_AUDIO_SOF(USBD_HandleTypeDef *pdev)
{
	...
	if(sof_count != 0 )
	{
		...
		/* EP IN 2: feedback */
#if 0
		if((haudio->rd_enables[0] > 0) && (haudio->EPIN_tx_flags[1] != 1)) // keep sending
		{
			if((haudio->EPIN_NSOF[1] & 0x1) == (fnsof_new & 0x1))
			{
				if(haudio->EPIN_tx_flags[1] == 255)
				{
					memset(&counter_feedback, 0, sizeof(precise_counter_t));
					get_UAC1_0_feedback_bytes(haudio->sample_rate_Hzs[0], (uint32_t*)bufFeedback);
				}
				else
				{
					uint32_t feedbackHz = get_audio_feedback_Hz_from_fifo_free(haudio->sample_rate_Hzs[0],
							  kfifo_get_free_space(&fifo_from_USB_playback),
							  fifo_from_USB_playback.size);
					get_UAC1_0_feedback_bytes(feedbackHz, (uint32_t*)bufFeedback);
				}
...
static uint8_t USBD_AUDIO_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
	...
	if (is_USB_playback_inited == 1 && epnum == AUDIO_SPEAKER_OUT_EP)
	{
...
#if 0 /* write to a local buffer first */
	    size_t bytesFreeBeforeWrite = kfifo_put(pfifo, haudio->bufDO, PacketSize);
	    if(bytesFreeBeforeWrite < PacketSize)
	    	++fifoOverflowCounter_USB_playback;
		// allow app to read when buffer is half filled
		if (haudio->rd_enables[0] == 0U)
		{
			if (kfifo_get_filled_size(pfifo) >= pfifo->half)
			{
				haudio->rd_enables[0] = 1U;
			}
		}
#else /* directly write to fifo*/
		...
		(void) USBD_LL_PrepareReceive(pdev, AUDIO_SPEAKER_OUT_EP,
				haudio->bufDO, AUDIO_OUT_PACKET_MAX);
		haudio->EPOUT_NSOF[0] = fnsof_new;
		// fire signal to USB data processor
//		osThreadFlagsSet(taskUSBPlayerLoader, USB_LOADER_SIGNAL_RECEIVED);
	}
